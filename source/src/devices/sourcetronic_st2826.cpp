...
const QMap<QString,DeviceParameterConstraint> SourceTronic_ST2826::_deviceParamMap = {
    {"C,x",DeviceParameterConstraint("C,x", 0.0, 0.0, READONLY)},
    {"ZTD",DeviceParameterConstraint("ZTD", 0.0, 0.0, READONLY)},
    {"F",DeviceParameterConstraint("F", 0.0, 0.0, READWRITE)}
};

SourceTronic_ST2826::SourceTronic_ST2826(QString _interfaceName, quint32 _baudRate, QWidget *parent)
    : ScpiDevice(_interfaceName,_baudRate,parent){
    init();
}

const QMap<QString,DeviceParameterConstraint> SourceTronic_ST2826::getDeviceParameterConstraints()const{
    return _deviceParamMap;
}

const QString SourceTronic_ST2826::getInterfaceName()const{
    return interfaceName;
}

void SourceTronic_ST2826::init(){
    MeasurementDevice::init(_deviceName, interfaceName,_deviceParamMap);
    // initialize scpiCommand("...");
}
void SourceTronic_ST2826::setOutputState(bool on){
    if (!_outputOn&&on){
...
QString SourceTronic_ST2826::translateMeas(QString paramName){
    QString scpiCommandString = "";
    if (paramName=="C,x"){
        //scpiCommandString = "FUNC:IMP CPQ";
       // scpiCommandString = "*TRG";
        emit scpiCommand("FUNC:IMP CpD");
        scpiCommandString = "FETC?";
    }
    if (paramName=="ZTD"){
        emit scpiCommand("FUNC:IMP ZTD");
        scpiCommandString = "FETC?";
    }
    if (paramName=="F"){
...
QString SourceTronic_ST2826::translateSet(QString paramName, double paramValue){
    QString scpiCommandString = "";
    if (paramName=="C"){
...
double SourceTronic_ST2826::translateInc(QString receivedString){
...
double SourceTronic_ST2826::translateInc1(QString receivedString){
    QString message = receivedString;
    //message.chop(0); // chop 2: the '\r' and the measurement unit
    //bool ok = false;
    QStringList wert1=message.split(",");
